...
function AppearancePopover({ settings, setSettings, onClose }: { settings: Settings; setSettings: React.Dispatch<React.SetStateAction<Settings>>; onClose: () => void }) { return <section role="dialog" aria-label="Reading appearance" className="fixed right-14 top-24 z-[80] w-72 rounded-2xl border border-[var(--reader-border)] bg-[var(--reader-panel)] p-5 text-[var(--reader-fg)] shadow-xl shadow-[var(--reader-shadow)]"><div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-semibold">Reading appearance</h2><button onClick={onClose} aria-label="Close reading appearance" className="reader-icon-button grid size-8 place-items-center rounded-full"><X size={16}/></button></div><AppearanceControls settings={settings} setSettings={setSettings}/></section>; }
function AppearanceControls({ settings, setSettings }: { settings: Settings; setSettings: React.Dispatch<React.SetStateAction<Settings>> }) { return <div className="space-y-5"><label className="block text-xs text-[var(--reader-fg)]">Text size <span className="float-right text-[var(--reader-muted)]">{settings.fontSize}px</span><input aria-label="Text size" className="mt-2 w-full accent-[var(--reader-accent)]" type="range" min="15" max="27" value={settings.fontSize} onChange={(event) => setSettings((value) => ({ ...value, fontSize: Number(event.target.value) }))}/></label><label className="block text-xs text-[var(--reader-fg)]">Line height <span className="float-right text-[var(--reader-muted)]">{settings.lineHeight.toFixed(1)}</span><input aria-label="Line height" className="mt-2 w-full accent-[var(--reader-accent)]" type="range" min="1.3" max="2.1" step=".1" value={settings.lineHeight} onChange={(event) => setSettings((value) => ({ ...value, lineHeight: Number(event.target.value) }))}/></label><div role="group" aria-label="Font family" className="grid grid-cols-2 gap-2"><button aria-pressed={settings.fontFamily === "serif"} onClick={() => setSettings((value) => ({ ...value, fontFamily: "serif" }))} className={`rounded-lg border border-[var(--reader-border)] p-2 reader-serif text-sm hover:bg-[var(--reader-hover)] ${settings.fontFamily === "serif" ? "bg-[var(--reader-accent-soft)] text-[var(--reader-accent-strong)]" : "text-[var(--reader-fg)]"}`}>Serif</button><button aria-pressed={settings.fontFamily === "sans"} onClick={() => setSettings((value) => ({ ...value, fontFamily: "sans" }))} className={`rounded-lg border border-[var(--reader-border)] p-2 text-sm hover:bg-[var(--reader-hover)] ${settings.fontFamily === "sans" ? "bg-[var(--reader-accent-soft)] text-[var(--reader-accent-strong)]" : "text-[var(--reader-fg)]"}`}>Sans</button></div><div role="group" aria-label="Reader theme" className="grid grid-cols-3 gap-2">{(["light", "paper", "dark"] as const).map((theme) => <button key={theme} aria-label={`${theme[0].toUpperCase()}${theme.slice(1)} theme`} aria-pressed={settings.theme === theme} onClick={() => setSettings((value) => ({ ...value, theme }))} className={`reader-theme-choice grid h-10 place-items-center rounded-lg border ${theme === "dark" ? "border-[#4b4d47] bg-[#272825] text-[#ded8cc]" : theme === "paper" ? "border-[#d8d0c2] bg-[#f3ead9] text-[#554f46]" : "border-[#d8d8d5] bg-[#fbfaf7] text-[#4f555b]"}`}>{theme === "dark" ? <Moon size={15}/> : <Sun size={15}/>}</button>)}</div><button aria-pressed={settings.mode === "scrolled-doc"} onClick={() => setSettings((value) => ({ ...value, mode: value.mode === "paginated" ? "scrolled-doc" : "paginated" }))} className="flex w-full items-center justify-center gap-2 rounded-lg border border-[var(--reader-border)] py-2 text-xs text-[var(--reader-fg)] hover:bg-[var(--reader-hover)]"><Columns2 size={14}/>{settings.mode === "paginated" ? "Paginated" : "Continuous scroll"}</button></div>; }
...
function MobileSheet({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <div className="fixed inset-0 z-[90] bg-[var(--reader-overlay)] sm:hidden"><section role="dialog" aria-modal="true" aria-label={title} className="absolute inset-x-0 bottom-0 max-h-[78vh] overflow-y-auto rounded-t-[24px] border-t border-[var(--reader-border)] bg-[var(--reader-panel)] p-5 text-[var(--reader-fg)]"><div className="mb-3 flex justify-between"><h2 className="reader-serif text-xl">{title}</h2><button onClick={onClose} aria-label={`Close ${title.toLowerCase()}`} className="reader-icon-button grid size-8 place-items-center rounded-full"><X size={17}/></button></div>{children}</section></div>; }
function SearchDialog({ toc, onNavigate, onClose }: { toc: TocItem[]; onNavigate: (href: string) => void; onClose: () => void }) { const [query, setQuery] = useState(""); const results = toc.filter((item) => item.label.toLowerCase().includes(query.toLowerCase())); return <div className="fixed inset-0 z-[100] grid place-items-start bg-[var(--reader-overlay)] px-4 pt-[12vh]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section role="dialog" aria-modal="true" aria-label="Search book" className="w-full max-w-xl rounded-2xl border border-[var(--reader-border)] bg-[var(--reader-panel)] text-[var(--reader-fg)] shadow-2xl shadow-[var(--reader-shadow)]"><div className="flex items-center gap-3 border-b border-[var(--reader-border)] px-5 text-[var(--reader-muted)]"><Search size={18}/><input aria-label="Search chapter titles" autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search chapter titles…" className="h-14 flex-1 bg-transparent text-sm text-[var(--reader-fg)] outline-none placeholder:text-[var(--reader-subtle)]"/></div><div className="max-h-72 overflow-y-auto p-2">{results.map((item) => <button key={item.href} onClick={() => onNavigate(item.href)} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm text-[var(--reader-fg)] hover:bg-[var(--reader-hover)]"><BookOpen size={15}/>{item.label}</button>)}</div></section></div>; }
