...
import type { AiChunk, AiSettings, Highlight, LocalBook, LocalCitation } from "@/shared/ipc";
...
export function ReaderShell({ book, onOpenSettings }: { book: LocalBook; onOpenSettings: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    const refreshAi = () => void Promise.all([window.marginReader.aiSettings.get(), window.marginReader.credentials.status()]).then(async ([nextSettings, credentials]) => {
      if (!active) return;
      if (nextSettings.provider === "gemini") { setAiConfiguration({ settings: nextSettings, ready: credentials.configured }); return; }
      const ollama = await window.marginReader.ollama.status();
      if (active) setAiConfiguration({ settings: nextSettings, ready: ollama.available && Boolean(nextSettings.ollamaModel && ollama.models.some((model) => model.name === nextSettings.ollamaModel)) });
    });
    refreshAi(); window.addEventListener("margin:ai-settings-changed", refreshAi);
    return () => { active = false; window.removeEventListener("margin:ai-settings-changed", refreshAi); };
  }, []);
...
function AiMargin({ book, provider, scope, setScope, answer, status, error, question, setQuestion, onAsk, onStop, onClose, onCitation, onOpenSettings }: { book: LocalBook; provider: "gemini" | "ollama"; scope: ReaderScope; setScope: (scope: ReaderScope) => void; answer?: Answer; status: "ready" | "streaming"; error: string; question: string; setQuestion: (value: string) => void; onAsk: (text: string) => void; onStop: () => void; onClose: () => void; onCitation: (citation: LocalCitation) => void; onOpenSettings: () => void }) {
