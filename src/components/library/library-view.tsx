...
export function LibraryView({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  return <main className="min-h-dvh bg-[#f4efe5] px-8 pb-16 pt-[52px] text-[#25231f]">
    <header className="mx-auto flex h-16 max-w-[1380px] items-center justify-between border-b border-[#d8d0c2]/80">
      <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-[#28313d] text-[#f7f1e6]"><BookOpen size={17}/></span><span className="text-[15px] font-semibold tracking-[.02em]">Margin Reader</span><span className="rounded-full border border-[#d5cdc0] px-2 py-1 text-[9px] uppercase tracking-[.12em] text-[#7a7369]">On this Mac</span></div>
      <div className="flex items-center gap-1"><button onClick={() => setSearchOpen(true)} className="grid size-10 place-items-center rounded-full hover:bg-white/45" aria-label="Search library"><Search size={16}/></button><button onClick={onOpenSettings} className="grid size-10 place-items-center rounded-full hover:bg-white/45" aria-label="Settings"><Settings size={16}/></button><button onClick={() => void upload()} className="ml-1 flex h-10 items-center gap-2 rounded-full bg-[#2c3541] px-5 text-sm font-medium text-white shadow-sm transition hover:bg-[#202832]"><Plus size={16}/> Add book</button><input ref={inputRef} className="sr-only" type="file" accept=".epub,application/epub+zip" onChange={(event) => void upload(event.target.files?.[0])}/></div>
    </header>
    <div className="mx-auto max-w-[1380px] pt-11">
      <div className="mb-10 flex items-end justify-between gap-8"><div className="animate-rise"><p className="small-caps mb-3 text-xs font-semibold text-[#7b7267]">Your private reading room</p><h1 className="reader-serif text-5xl leading-[1.04] tracking-[-.03em]">Pages worth returning to.</h1><p className="mt-4 max-w-xl text-sm leading-relaxed text-[#70695f]">Your books, notes, and search index stay on this Mac. Add a DRM-free EPUB and keep reading offline.</p></div>
        <div onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void upload(event.dataTransfer.files[0]); }} className={`flex min-h-20 min-w-[330px] items-center gap-4 rounded-2xl border border-dashed px-5 py-4 transition ${dragging ? "border-[#536b88] bg-white/75" : "border-[#c9c0b2] bg-[#faf7f0]/50"}`}><CloudUpload className="text-[#6f7d8c]" size={22}/><div><p className="text-sm font-medium">Drop a DRM-free EPUB</p><p className="mt-0.5 text-xs text-[#7a746b]">Up to 50 MB · copied into your library</p></div></div>
      </div>
